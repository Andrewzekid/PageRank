...
This process should repeat until no PageRank value changes by more than 0.001 between the current rank values and the new rank values.
    ...
    #Create page_ranks
    page_ranks = {key:1/N for key in corpus.keys()}
    ...
    while max_diff > 0.001:
        ...
        for page in corpus.keys():
            ...
            for candidate_page,links in corpus.items():
                if page in links:
            ...
            #Calculate PR - loop through every page i 
            PR = ((1 - damping_factor)/N) + (damping_factor*sum([(page_ranks[i])/(len(corpus[i])) for i in linked_pages]))
        ...
        print(f"Starting PR values are: {start_vals} and ending values are {end_vals} with difference {max_diff}")
        print(f"Current page ranks list is: {page_ranks}")
        print("==============================================================================================================")
